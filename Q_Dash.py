...
data_projects_distribution= data_bar_input.groupby("Projekt")["Input"].sum()
...
with st.expander("Total Data per selection"):
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.header("Input for selected week(s) per project")
        st.bar_chart(data_projects_distribution)

    with col2:
        st.header("Dataset")
        st.write(data_projects_distribution)
